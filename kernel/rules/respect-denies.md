...
The user's `.env` files and credential paths are denied for a reason. To work with API keys in scripts:
1. Run `~/.claude/scripts/list-env-keys.sh [pattern]` to discover the **names** of available credential env vars (never values).
2. Reference the variable by name in your code: `os.environ['GEMINI_API_KEY']`.
3. The user runs the script and provides values via their shell environment, not via you reading them.
